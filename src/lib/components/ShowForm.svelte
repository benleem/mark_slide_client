<script lang="ts">
    ...
    const handleSubmit = async() => {
        formLoading = true
        const response = mode === "show-add" ?  await postShow(showFormData) : await patchShow(showFormData, show.id)
...
{:else}
     <form class="p-5 flex flex-col gap-2 bg-[#030303] rounded-md border-[1px] border-gray-600" novalidate autocomplete="off" on:submit|preventDefault={() => handleSubmit()}>
        <div class="pb-2 flex justify-between border-b-[1px] border-gray-600">
            <h3 class="text-lg">{mode === "show-add" ? "Add show" : "Edit show details"}</h3>
            <button 
                on:click={() => $modal = {
                    active:false, 
                    type:null, 
