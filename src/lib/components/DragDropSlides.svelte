<script lang="ts">
    import  type {CreateSlideData, Slide, DeleteSlideParams, UpdateSlideData, TestSlide } from "$lib/models/slides";
    ...
    function handleFinalize(event: CustomEvent<DndEvent<TestSlide>>) {
        ...
        let currentSlide = $showSlides.findIndex((slide => slide.id === selectedSlide.id))
    ...
    async function createNewSlide() {
        let createSlideBody: TestSlide = {id: uuidv4(), content: "", index_number: $showSlides.length};
...
<div class="h-full overflow-scroll w-full max-w-xs pr-2">
    <div
        ...
        use:dndzone="{{items: $showSlides, flipDurationMs, type:'columns', dropTargetStyle:{}}}" 
    ...
    >
        {#each $showSlides as slide(slide.id)}
            ...
            >
                <div
                    style={`border-color: ${slide && selectedSlide && slide.id === selectedSlide.id ? "#eab308" : "white"};`}
                    class="p-2 mb-2 border-2 border-white aspect-video overflow-hidden"
                >
                    <div class="prose prose-invert w-full prose-table:w-max scale-50 origin-top"> 
    ...
    >
        <span class="material-symbols-outlined text-3xl group-hover:text-blue-500 transition-colors duration-200 ease-in-out">
            add
        </span>
    </button>
</div>
